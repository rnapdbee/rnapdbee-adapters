...
from adapters.exceptions import InvalidSvgError, RegexError, ThirdPartySoftwareError
from adapters.tools.utils import run_external_cmd
from adapters.visualization.model import SYMBOLS, Model2D, Residue, SymbolType
...
class PseudoViewerDrawer:
    ...
    def parse_strands(self) -> None:
        ...
        for strand_index, strand in enumerate(self.data.strands):
            strand_name, structure, sequence = (
                strand.name,
                strand.structure,
                strand.sequence,
            )
            if self.has_no_pairs(structure):
                raise ThirdPartySoftwareError(
                    f"No pairs in strand {strand_name}, PseudoViewer image cannot be drawn"
                )
            ...
            modified_sequence.append(
                f"{self.get_letter_sequence(strand_index, len(sequence))}\n"
            )
            modified_structure.append("\n")
    ...
    def generate_pseudoviewer_svg(self) -> None:
        with TemporaryDirectory() as directory:
            with NamedTemporaryFile(
                "w+", dir=directory, suffix=".seq"
            ) as seqeunce_file:
                with NamedTemporaryFile(
                    "w+", dir=directory, suffix=".str"
                ) as structure_file:
    ...
    def remove_javascript(
        self, root: ET.Element, residues_elements: List[ET.Element]
    ) -> None:
