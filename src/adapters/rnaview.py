...
class RNAViewAdapter:
    ...
    RNAVIEW_REGEX = re.compile("\\s*(\\d+)_(\\d+),\\s+(\\w):\\s+(-?\\d+)\\s+(\\w+)-(\\w+)\\s+" +
    ...
    def append_residues_from_pdb_using_rnaview_indexing(self, pdb_content: str) -> None:
        ...
        for line in pdb_content.splitlines():
            if line.startswith(self.ATOM) or line.startswith(self.HETATM):
                ...
                icode = None if line[self.ICODE_INDEX].strip() == '' else line[self.ICODE_INDEX]
                ...
                if str(residue) not in potential_residues:
                    potential_residues[str(residue)] = RNAViewAdapter.PotentialResidue(residue, None, None, None)
